...
class DOLGFusionModule(nn.Module):
    ...
    def forward(self, local_feat, global_feat):
        ...
        cosine = torch.einsum('bc,bc->b', local_feat, global_feat).unsqueeze(-1) / norm_g
        proj = cosine * global_feat
        orth = local_feat - proj

        cat = torch.cat([global_feat.unsqueeze(-1), orth.unsqueeze(-1)], dim=-1)
