...
def train_one_epoch(train_loader, model, ema, loss_fn, optimizer, scaler, epoch, args):
    ...
    for labels, images in progress:
        ...
        enqueue_idx = torch.arange(labels.size(0), labels.size(0) * 2)

        q_image = images[0].cuda(args.gpu, non_blocking=True)
        k_image = images[1].cuda(args.gpu, non_blocking=True)
        labels = labels.cuda(args.gpu, non_blocking=True)
        labels = torch.tile(labels, dims=(args.ncrops,))

        with torch.cuda.amp.autocast():
            q_embeddings = model(q_image)

            with torch.no_grad():
                copy_params(model, ema, m=0.99)
                k_embeddings = ema(k_image)

            embeddings = torch.cat([q_embeddings, k_embeddings], dim=0)
            loss = loss_fn(embeddings, labels, enqueue_idx=enqueue_idx)
