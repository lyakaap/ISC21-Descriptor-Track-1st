...
class ISCNet(nn.Module):
    ...
    def __init__(self, backbone, fc_dim=256):
        super().__init__()
...
class RandomOverlayImage(BaseTransform):
    ...
    def apply_transform(
        ...
        return overlay_image(
            background,
            overlay=image,
            opacity=random.uniform(0.0, 1.0),
            overlay_size=random.uniform(0.1, 0.5),
            x_pos=random.uniform(0.0, 0.5),
            y_pos=random.uniform(0.0, 0.5),
            metadata=metadata,
        )
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    del vit.head
    # backbone = timm.create_model('tf_efficientnetv2_s_in21ft1k', features_only=True, pretrained=True, out_indices=[2])
    # vit = timm.create_model('swin_base_patch4_window7_224', pretrained=False)
    # vit.patch_embed = HybridEmbed(backbone, img_size=224, patch_size=1, embed_dim=vit.embed_dim)
