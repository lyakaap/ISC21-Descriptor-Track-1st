...
def extract2(args):
    ...
    preprocesses = [
        transforms.Resize((int(args.input_size + args.input_size * 1.5), int(args.input_size + args.input_size * 1.5))),
        transforms.FiveCrop(args.input_size),  # this is a list of PIL Images
    ...
    def calc_feats(loader):
        ...
        for _, image in tqdm(loader, total=len(loader)):
            ...
            with torch.no_grad(), torch.cuda.amp.autocast():
                ...
                f = model(image.view(bs * ncrops, c, h, w))
            feats.append(f.cpu().numpy())
        feats = np.concatenate(feats, axis=0)
        return feats.astype(np.float32)
    ...
    if 'q' in args.target_set:
        ...
        out = f'{ver}/extract/five_crop.h5'
        with h5py.File(out, 'w') as f:
            f.create_dataset('query', data=query_feats)
            f.create_dataset('reference', data=reference_feats)
            f.create_dataset('query_ids', data=query_ids)
        ...
        query_feats = query_feats.reshape(-1, 5, 256).mean(axis=1)
        query_feats /= np.linalg.norm(query_feats, 2, axis=1, keepdims=True)
        reference_feats = reference_feats.reshape(-1, 5, 256).mean(axis=1)
        reference_feats /= np.linalg.norm(reference_feats, 2, axis=1, keepdims=True)
...
def extract3(args):
    ...
    preprocesses = [
        transforms.Resize((int(args.input_size + args.input_size * 1.5), int(args.input_size + args.input_size * 1.5))),
        transforms.TenCrop(args.input_size),  # this is a list of PIL Images
        transforms.Lambda(lambda crops: torch.stack([transforms.ToTensor()(crop) for crop in crops])),
        transforms.Normalize(mean=backbone.default_cfg['mean'], std=backbone.default_cfg['std'])
    ]
    ...
    if 'q' in args.target_set:
        ...
        query_feats = query_feats.reshape(-1, 10, 256).mean(axis=1)
        query_feats /= np.linalg.norm(query_feats, 2, axis=1, keepdims=True)
        reference_feats = reference_feats.reshape(-1, 10, 256).mean(axis=1)
        reference_feats /= np.linalg.norm(reference_feats, 2, axis=1, keepdims=True)
