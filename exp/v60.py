...
class NTXentLoss(GenericPairLoss):
    def __init__(self, temperature=0.07, margin=0.3, **kwargs):
        super().__init__(mat_based_loss=False, **kwargs)
        self.temperature = temperature
        self.margin = margin
        self.add_to_recordable_attributes(list_of_names=["temperature", "margin"], is_stat=False)
    ...
    def _compute_loss(self, pos_pairs, neg_pairs, indices_tuple):
        ...
        if len(a1) > 0 and len(a2) > 0:
            ...
            pos_pairs = (pos_pairs.unsqueeze(1) - self.margin) / self.temperature
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    loss_fn = NTXentLoss(temperature=args.temperature, margin=args.margin)
