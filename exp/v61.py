...
def main_worker(gpu, ngpus_per_node, args):
    ...
    loss_fn_rq = losses.ContrastiveLoss(pos_margin=args.pos_margin, neg_margin=args.neg_margin)
    loss_fn_rq = losses.CrossBatchMemory(loss_fn_rq, embedding_size=256, memory_size=args.memory_size)
    loss_fn_rq = pml_dist.DistributedLossWrapper(loss=loss_fn_rq, device_ids=[args.rank])

    loss_fn_qr = losses.ContrastiveLoss(pos_margin=args.pos_margin, neg_margin=args.neg_margin)
    loss_fn_qr = losses.CrossBatchMemory(loss_fn_qr, embedding_size=256, memory_size=args.memory_size)
    loss_fn_qr = pml_dist.DistributedLossWrapper(loss=loss_fn_qr, device_ids=[args.rank])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(train_loader, model, loss_fn_rq, loss_fn_qr, optimizer, scaler, epoch, args)
...
def train_one_epoch(train_loader, model, loss_fn_rq, loss_fn_qr, optimizer, scaler, epoch, args):
    ...
    for labels, images in progress:
        ...
        enqueue_idx_rq = torch.arange(labels.size(0), labels.size(0) * 2)
        enqueue_idx_qr = torch.arange(labels.size(0))
        ...
        with torch.cuda.amp.autocast():
            ...
            loss = (
                loss_fn_rq(embeddings, labels, enqueue_idx=enqueue_idx_rq)
                + loss_fn_qr(embeddings, labels, enqueue_idx=enqueue_idx_qr)
            )
